 ...
 *
 * The `<meta name="api-token">` tag carries the LOOPBACK file-token
 * (not a JWT), so it must be sent as `X-Auth-Token` — which is the
 * header the server's requireAuth middleware matches against the
 * file-token. Sending it as `Authorization: Bearer` would make the
 * server try to verify it as a JWT and fail.
 *
 * Callers that already signed in with a real JWT pass it via the
 * Authorization header in `init.headers` — we don't clobber that.
 */
export async function apiFetch(url: string, init?: RequestInit): Promise<Response> {
  const token = getApiToken();
  const headers = new Headers(init?.headers);

  if (token && !headers.has("Authorization") && !headers.has("X-Auth-Token")) {
  ...
  const res = await fetch(resolveUrl(url), { ...init, headers, credentials: "include" });

  // Vault in-memory key is wiped on server restart (tsx watch). When
  // a call fails with "vault locked" (any 4xx/5xx whose body mentions
  // it), silently re-run /auth/login with the cached passphrase so
  // the user doesn't see stale data after a dev reload.
  if (!res.ok) {
    const cached = sessionStorage.getItem("daemora_vault_pass");
    if (cached) {
      const body = await res.clone().text();
      const vaultLocked = /vault\s*is\s*locked/i.test(body);
      if (vaultLocked) {
        const relogin = await fetch(resolveUrl("/auth/login"), {
          method: "POST",
          headers: {
        ...
        if (relogin.ok) {
          return fetch(resolveUrl(url), { ...init, headers, credentials: "include" });
        }
      }
    }
  }
