...
export function TaskDetail() {
  ...
  useEffect(() => {
    ...
    // 1. Fetch once. 2. If running, attach SSE + poll fallback.
    //    The previous version checked `task?.status` in closure, which
    //    was always null at this point and skipped both branches.
    fetchTask().then((initial) => {
      if (!active || !initial) return;
      const isRunning = initial.status === "running" || initial.status === "pending";
      if (!isRunning) return;

      // SSE for live tool events. Server emits `task:state` with the
      // full task on completion/failure (NOT `task:completed`/`task:failed`
      // — those names predate the unified `task:state` event).
      es = new EventSource(apiStreamUrl(`/api/tasks/${id}/stream`));
      es.addEventListener("tool:after", () => fetchTask());
      es.addEventListener("tool:before", () => fetchTask());
      es.addEventListener("task:state", (e) => {
        try {
          const payload = JSON.parse((e as MessageEvent<string>).data) as { status?: string };
          if (payload.status === "completed" || payload.status === "failed") {
            // Final status reached — pull the full record (with cost,
            // duration, tool calls) and stop streaming.
            fetchTask();
            es?.close();
            es = null;
          }
      ...
      es.onerror = () => { /* keep silent — SSE auto-reconnects */ };

      // Belt-and-braces polling fallback — covers the case where the
      // task already finished by the time SSE attaches (60s lifetime
      // of activeTasks in the server).
      pollHandle = setInterval(async () => {
        const fresh = await fetchTask();
            ...
            <TabsContent value="tools" className="p-6">
              {!task.toolCalls || task.toolCalls.length === 0 ? (
                ...
                <div className="space-y-3">
                  {task.toolCalls.map((tc: any, index: number) => (
                    <div
                      key={tc.id ?? index}
                      className={`bg-slate-800/50 border rounded-lg p-4 ${tc.error ? "border-red-500/20" : "border-slate-700"}`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <div className={`w-5 h-5 rounded flex items-center justify-center ${tc.error ? "bg-red-500/20" : "bg-[#00d9ff]/20"}`}>
                            <span className="text-[9px] font-mono font-bold text-gray-300">#{tc.id ?? index + 1}</span>
                          </div>
                          <span className="font-bold text-white font-mono text-sm">{tc.toolName ?? tc.tool ?? "unknown"}</span>
                          {tc.error && (
                            <Badge variant="outline" className="text-red-400 border-red-500/30 text-[9px] font-mono">ERROR</Badge>
                          )}
                        </div>
                        <span className="text-[10px] font-mono text-gray-500">{tc.durationMs ?? tc.duration ?? 0}ms</span>
                      </div>
                      {(tc.args || tc.params) && (
                        <div className="mb-2">
                          ...
                          <pre className="font-mono text-[11px] bg-slate-900/50 p-2 rounded border border-slate-800 text-gray-400 overflow-x-auto max-h-24 overflow-y-auto">
                            {JSON.stringify(tc.args ?? tc.params, null, 2)}
                          </pre>
                        </div>
                      )}
                      {(tc.result || tc.output_preview || tc.error) && (
                        <div>
                          <div className="text-[9px] font-mono text-gray-600 uppercase mb-1">{tc.error ? "Error" : "Output"}</div>
                          <pre className={`font-mono text-[11px] bg-slate-900/50 p-2 rounded border border-slate-800 overflow-x-auto max-h-32 overflow-y-auto ${tc.error ? "text-red-400" : "text-gray-400"}`}>
                            {tc.error ?? JSON.stringify(tc.result ?? tc.output_preview, null, 2)}
                          </pre>
                        </div>
                      )}
                    </div>
