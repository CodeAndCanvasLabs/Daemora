...
export function Channels() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<ChannelDef | null>(null);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await apiFetch(`/api/channels/${encodeURIComponent(deleteTarget.name)}`, {
        method: "DELETE",
      });
      if (!res.ok) {
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    {ch.configured && (
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#ff4458]/10 border border-[#ff4458]/30 text-[#ff4458] text-sm font-medium hover:bg-[#ff4458]/20 transition-all cursor-pointer"
                      >
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This stops the channel if running and wipes its secrets from the vault. You can reconfigure it later — the channel itself isn't uninstalled.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
