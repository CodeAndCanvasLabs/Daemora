...
export function saveJob(job) {
  run(
    `INSERT INTO cron_jobs (
      ...
      delivery_targets, delivery_preset_id,
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      delivery_targets=$delTargets, delivery_preset_id=$delPresetId,
      ...
      updated_at=$updatedAt`,
    {
      ...
      $delTargets: job.delivery?.targets ? JSON.stringify(job.delivery.targets) : null,
      $delPresetId: job.delivery?.presetId || null,
...
function _rowToJob(row) {
  return {
    ...
    delivery: {
      ...
      targets: row.delivery_targets ? JSON.parse(row.delivery_targets) : null,
      presetId: row.delivery_preset_id || null,
    },
